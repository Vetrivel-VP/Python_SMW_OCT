...
def add():
    #get() : Reads data in string. 
    #eval() : Typecast it from string to Int or Float
    result = eval(a_text.get()) + eval(b_text.get())

    lbl_ans.configure(text=f'Answer : {a_text.get()} + {b_text.get()} = {result}')
    #Answer : 5 + 5 = 10
...
# Text : Label()
lbl_heading = tk.Label(frame, text="Addition of two numbers", bg="#3e646c", fg="white")
...
#  Button : Button()
add_btn = tk.Button(frame, text="Add", padx=25, pady=5, bg="#263d42", fg="white", command=add)
add_btn.place(x=50, y= 170)

#  Button : Button()
clear_btn = tk.Button(frame, text="Clear", padx=25, pady=5, bg="#263d42", fg="white", comman=clear_data)
clear_btn.place(x=170, y= 170)

#Label a : ENter the value of a:
lbl_ans = tk.Label(frame, text="", bg="#3e646c", fg="white")
lbl_ans.place(x = 80, y=210)
